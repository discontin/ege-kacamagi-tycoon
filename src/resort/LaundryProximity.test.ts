...
import { initialResort, TOWEL_RACK, DIRTY_BASKET } from './data';
...
describe('laundry object proximity', () => {
  it.each([{ x: TOWEL_RACK.x + .5, y: 45.5 }, { x: TOWEL_RACK.x, y: 45.5 }, { x: TOWEL_RACK.x + .9, y: 45.5 }, { x: TOWEL_RACK.x + .9, y: 45.2 }])('takes towels automatically at shelf edge %j', p => {
  ...
  it.each([{ x: DIRTY_BASKET.x, y: DIRTY_BASKET.y + 1 }, { x: DIRTY_BASKET.x - 1, y: DIRTY_BASKET.y }, { x: DIRTY_BASKET.x + .5, y: DIRTY_BASKET.y + 1 }])('deposits dirty towels automatically at basket edge %j', p => {
  ...
  it('pauses a transfer outside range and resumes the saved job at another shelf edge', () => {
    ...
    Object.assign(s.state.player, { x: TOWEL_RACK.x + .5, y: 45.5 }); advance(s, .2);
    ...
    const loaded = new ResortSimulation(structuredClone(s.state)); Object.assign(loaded.state.player, { x: TOWEL_RACK.x, y: 45.5 }); advance(loaded, .8);
  ...
  it('workers can transfer from the side rather than walking back to the old fixed square', () => {
    ...
    expect(s.startTask(s.area('cleanTake')!, w.id)).toBe(true); Object.assign(w, { x: TOWEL_RACK.x + .9, y: 45.5, path: [] }); advance(s, 1);
    expect(w.bag.clean).toBe(1); expect(w.x).toBe(TOWEL_RACK.x + .9); expect(w.y).toBe(45.5);
  });
});
