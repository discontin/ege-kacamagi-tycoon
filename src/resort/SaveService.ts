...
const kinds = ['checkin', 'cleanRoom', 'cleanFloor', 'cleanBathroom', 'restockRoom', 'dirtyDrop', 'cleanTake', 'poolCheckin', 'cleanSeat', 'poolStock', 'cleanPool', 'prepareDrink', 'deliverDrink', 'poolDirtyDrop', 'poolDirtyTake', 'poolCleanTake', 'restockSeat', 'laundryDirtyTake', 'machineLoad', 'machineUnload', 'laundryCleanDrop', 'discardItem'];
const num = (v: unknown) => typeof v === 'number' && Number.isFinite(v) && v >= 0;
const point = (v: any) => v && num(v.x) && typeof v.y === 'number' && Number.isFinite(v.y) && v.y >= -3 && v.x <= 43 && v.y <= 51;
const actor = (v: any) => point(v) && typeof v.id === 'string' && Array.isArray(v.path) && v.path.every(point) && ((v.drink === undefined || typeof v.drink === 'boolean') && (v.heldProduct === undefined || ['lemonade', 'icecream'].includes(v.heldProduct))) && (v.carryingWashed === undefined || typeof v.carryingWashed === 'boolean');
...
export class ResortSaveService {
  ...
  load() {
    ...
    try { const raw = this.storage.getItem(RESORT_SAVE_KEY); if (!raw) return { state: initialResort(), recovered: false, existing: false }; const v: any = JSON.parse(raw); if (Array.isArray(v?.seats) && v.seats.length === 4) for (const d of SEAT_DEFS.slice(4)) v.seats.push({ id: d.id, open: false, dirty: false, towel: false }); if (Array.isArray(v?.facilities)) for (const f of v.facilities) if (f?.kind === 'room' && f.level === 3) f.level = 2; if (!validResort(v)) throw new Error('Invalid save'); return { state: v, recovered: false, existing: true }; }
