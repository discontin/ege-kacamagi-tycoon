...
describe('physical sheet laundry cycle', () => {
  it('collects dirty sheet and towel, washes both, and consumes a clean sheet when remaking the bed', () => {
    ...
    stand(s, 'laundryDirtyTake'); advance(s, .7);
    for (let i = 0; i < 2; i++) { stand(s, 'machineLoad'); advance(s, 5); stand(s, 'machineUnload'); advance(s, .7); stand(s, 'laundryCleanDrop'); advance(s, .7); }
    ...
    stand(s, 'cleanTake'); advance(s, 1.2); expect(s.state.player.bag.cleanSheets).toBe(2); expect(linenCount(s.state.player.bag)).toBe(3);
  ...
  it('holds a washed sheet when its output shelf fills, without losing or duplicating it', () => {
    ...
    expect(l.washingKind).toBe('sheet'); Object.assign(s.state.player, { x: 19, y: 48 }); l.cleanSheets = 24; advance(s, 5); expect(l.remaining).toBe(0); expect(l.cleanSheets).toBe(24);
    stand(s, 'machineUnload'); advance(s, .7); stand(s, 'laundryCleanDrop'); advance(s, 1); expect(s.state.player.bag.cleanSheets).toBe(1); expect(l.cleanSheets).toBe(24);
  ...
  it('preserves bag sheets and a partial washing cycle across pause and save/load', () => {
    ...
    const loaded = new ResortSimulation(save.load().state); expect(loaded.state).toEqual(s.state); loaded.state.settings.paused = false; advance(loaded, 4);
