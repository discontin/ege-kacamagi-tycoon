...
export class ResortWorld {
  ...
  private animate = (now: number) => {
    ...
    for (const [id, linen] of this.bedLinen) { const room = this.sim.facility(id), task = s.tasks.find(t => t.target === id && t.kind === 'cleanRoom'); linen.root.visible = true; linen.update(room.dirty ? task ? cleaningProgress(task.remaining, task.total) : 0 : 1); }
