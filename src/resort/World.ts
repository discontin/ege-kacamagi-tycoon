...
export class ResortWorld {
  ...
  private animate = (now: number) => {
    ...
    for (const [id, linen] of this.bedLinen) { const task = s.tasks.find(t => t.target === id && t.kind === 'cleanRoom'); linen.root.visible = !this.sim.facility(id).needsSheet || this.sim.facility(id).dirty; linen.update(this.sim.facility(id).dirty ? task ? cleaningProgress(task.remaining, task.total) : 0 : 1); }
